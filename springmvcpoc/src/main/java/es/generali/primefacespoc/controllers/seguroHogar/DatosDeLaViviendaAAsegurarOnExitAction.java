...
public class DatosDeLaViviendaAAsegurarOnExitAction extends OnExitActionBase<SeguroViviendaBean> {
	...
	@RequestMapping(value="/datosDeLaViviendaAAsegurar/submit")
	public ModelAndView submit(@ModelAttribute("model") SeguroViviendaBean model, BindingResult result, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		validationService.validate(model, result, DatosDeLaViviendaAAsegurar.class);
		...
		if (result.hasErrors()) {
			...
			return new ModelAndView("redirect:/es/seguroHogar/datosDePago/entry?execution=" + flowScope.getExecutionId());
		}
	}
